...
export const updateItems = async (params: TransactionWriteDynamoParams) => {
  ...
  const currentBatchToUpdate: TransactWriteItemsCommandInput = {
    TransactItems: [],
  };
  ...
  for (const transactItem of TransactItems) {
    ...
    currentBatchToUpdate.TransactItems?.push(itemToUpdate as TransactWriteItem);

    if (
      currentBatchToUpdate.TransactItems &&
      currentBatchToUpdate.TransactItems.length % 25 === 0
    ) {
      await documentClient.transactWrite(currentBatchToUpdate);
